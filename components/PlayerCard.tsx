...
const PlayerCard: React.FC<PlayerCardProps> = ({ player, onUpdate, onDelete, isInBattle, onToggleBattle, maxLevel, lang }) => {
  ...
  const QuickAdjustButtons = ({ onAdjust }: { onAdjust: (val: number) => void }) => (
    <div className="flex justify-between gap-1 mt-3 w-full">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-slate-900/50 rounded-lg p-3 flex flex-col items-center border border-slate-700/50">
          ...
          <div className="flex items-center justify-center gap-2">
            <button onClick={() => adjustLevel(-1)} className="p-1 hover:bg-slate-700 rounded text-slate-500 transition-colors"><ChevronDown size={18} /></button>
            <span className="text-3xl font-black text-white leading-none min-w-[1.2ch] text-center">{player.level}</span>
            <button onClick={() => adjustLevel(1)} className="p-1 hover:bg-slate-700 rounded text-slate-500 transition-colors"><ChevronUp size={18} /></button>
          </div>
          <QuickAdjustButtons onAdjust={adjustLevel} />
        </div>
